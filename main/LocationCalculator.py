...
def least_median_square_estimate(loc_range_diff_info: list, depth: float, max_iters: int) -> list:
    ...
    for subset in subsets:
        ...
        sensor_location = tdoa_multilateration_from_multiple_segments_gauss_newton(anchor_locations, range_diffs, init_soln)
...
def perform_robust_multilateration(loc_range_diff_info, depth):
    ...
    for i in range(num_segment):
        ...
        estimated_location = tdoa_multilateration_from_single_segement_gauss_newton(locations, range_diffs, initial_soln)
...
def estimate_location(sound_speed: float, depth: float, beacon_signals: list) -> list:
    loc_range_diff_info = extract_tdoa_info(sound_speed, beacon_signals)

    if loc_range_diff_info:
        # estimated_locations = perform_robust_multilateration(loc_range_diff_info, depth)
        # estimated_location = tdoa_multilateration_from_all_segments_gauss_newton(loc_range_diff_info, depth)
        robust_location = least_median_square_estimate(loc_range_diff_info, depth, 400)
